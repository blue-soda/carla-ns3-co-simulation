    ...
    const AllocationInfo& allocationInfo)
{
    ...
    // 新增：定义资源参数默认值（避免无效配置）
    const uint32_t MIN_PSCCH_RBS = 2;       // PSCCH最小RB数（3GPP标准）
    const uint32_t MIN_PSSCH_RBS = 1;       // PSSCH最小RB数
    const uint32_t MIN_SYM_LENGTH = 2;      // 最小符号长度
    const int32_t MIN_SYM_START = 0;        // 最小符号起始索引
    ...
    for (; itTxOpps != selectedTxOpps.cend(); ++itTxOpps)
    {
        ...
        // 强制子信道长度不小于最小值，避免无可用RB
        slotAlloc.slPsschSubChLength = (itTxOpps->slSubchannelLength > 0) ? itTxOpps->slSubchannelLength : MIN_PSSCH_RBS;
