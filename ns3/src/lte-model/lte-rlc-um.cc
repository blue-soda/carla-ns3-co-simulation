...
LteRlcUm::LteRlcUm()
    : m_maxTxBufferSize(99 * 1024 * 1024),
      m_txBufferSize(0),
      m_sequenceNumber(0),
      m_vrUr(0),
      m_vrUx(0),
...
LteRlcUm::DoReceiveNrSlRlcPdu(NrSlMacSapUser::NrSlReceiveRlcPduParameters rxPduParams)
{
    ...
    NS_LOG_LOGIC("NR SL RLC header: " << rlcHeader);
    SequenceNumber10 seqNumber = rlcHeader.GetSequenceNumber();
    std::cout << "[NRSL_RLC_RX] rnti=" << m_rnti << " lcid=" << static_cast<uint32_t>(m_lcid)
              << " srcL2Id=" << rxPduParams.srcL2Id << " dstL2Id=" << rxPduParams.dstL2Id
              << " sn=" << seqNumber.GetValue() << " fi=" << static_cast<uint32_t>(rlcHeader.GetFramingInfo())
              << " size=" << rxPduParams.p->GetSize() << std::endl;
        ...
        (((m_vrUh - m_windowSize) <= seqNumber) && (seqNumber < m_vrUr)))
    {
        ...
        std::cout << "[NRSL_RLC_DROP] rnti=" << m_rnti << " lcid=" << static_cast<uint32_t>(m_lcid)
                  << " srcL2Id=" << rxPduParams.srcL2Id << " dstL2Id=" << rxPduParams.dstL2Id
                  << " sn=" << seqNumber.GetValue() << " vrUr=" << m_vrUr.GetValue()
                  << " vrUh=" << m_vrUh.GetValue() << std::endl;
        rxPduParams.p = nullptr;
        return;
    }
    else
