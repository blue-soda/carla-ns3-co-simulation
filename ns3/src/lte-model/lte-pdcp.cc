...
LtePdcp::DoReceiveNrSlPdcpPdu(Ptr<Packet> p)
{
    ...
    std::cout << "[NRSL_RLC_TO_PDCP] rnti=" << m_rnti << " lcid=" << static_cast<uint32_t>(m_lcid)
              << " srcL2Id=" << m_srcL2Id << " dstL2Id=" << m_dstL2Id << " size=" << p->GetSize()
              << " pdcpSn=" << pdcpHeader.GetSequenceNumber() << " sduType=" << static_cast<uint32_t>(sduType)
              << std::endl;
